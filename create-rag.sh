...
template_s3_url="https://e2e-rag-deployment-516199269042-us-east-1.s3.amazonaws.com/templates/main-template-out.yml"
source_docs_s3_bucket="ofac-rag-docs"
source_doc_local_dir="data"
...
list_resources_page() {
  ...
  if [[ -n "$token" ]]; then
    aws cloudformation list-stack-resources --region "$region" \
...
expand_stack() {
  ...
  while :; do
    if [[ -n "$next" ]]; then
      out=$(aws cloudformation list-stack-resources --stack-name "$stack" --starting-token "$next")
    else
      out=$(aws cloudformation list-stack-resources --stack-name "$stack")
    fi
...
main() {
  ...
  aws s3api create-bucket --bucket $source_docs_s3_bucket --region $region
  aws s3 cp ./$source_doc_local_dir s3://$source_docs_s3_bucket --recursive --region $region

  # Capabilities — KB stacks almost always need IAM; include NAMED_IAM in case roles have explicit names
